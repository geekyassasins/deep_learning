...
def atomic_test(df, signal, num_subjects, num_segments, target, percentage_test):
    ...
    data_zscore = []
   # print(df['Split'])
    #df['Split'] = df['Split'].astype('|S80')
    #dt=df.dtypes
    #print(df) 
    for i, row in df.iterrows():
        if target > round(np.all(num_subjects) * percentage_test) and target < 0:
    ...
    long = len(data_train)
    #print((long))
    while ((max(num_segments) % (len(data_train)+1)) != 0) and counter < int(max(num_segments) / long):
        counter += 1
    ...
    rand_candidates = np.random.random_integers(low=0, high=len(data_train) -1, size=(len(data_train),))
    #print(len(rand_candidates))
    for i in range(0, len(rand_candidates)):
        index = rand_candidates[i]
        if len(data_train_candidate)>index :
